...
const $ = new Env('途虎app')
$.KEY_sign = 'tuhuapp'
$.signKeyTUHU = 'tuhu_usersignKeyTUHU'
...
if (isGetCookie) {
  !(async () => {
    ...
    if ($.setdata(JSON.stringify(session), $.signKeyTUHU)) {
      $.subt = `获取会话: 成功!`
    } else {
      $.subt = `获取会话: 失败!`
    }
...
async function key() {
  return new Promise((resolve) => {
    ...
    signbody    = JSON.parse($.getdata($.signKeyTUHU)).body;
    signheaders = JSON.parse($.getdata($.signKeyTUHU)).headers;

    url_t = JSON.parse($.getdata($.signKeyTUHU)).url;
    const url = { 
