...
if (isGetCookie) {
  !(async () => {
    ...
    if ($.setdata(JSON.stringify(session), $.signKeysgs)) {
      $.subt = `获取会话: 成功!`
    } else {
      $.subt = `获取会话: 失败!`
    }
...
async function sign() {
  return new Promise((resolve) => {
    ...
    signbody    = JSON.parse($.getdata($.signKeysgs)).body;
    signheaders = JSON.parse($.getdata($.signKeysgs)).headers;

    url_t = JSON.parse($.getdata($.signKeysgs)).url;
    const url = { 
