...
if (isGetCookie) {
  !(async () => {
    ...
    if ($.setdata(JSON.stringify(session), $.signKeyTU)) {
      ...
      session.token && console.log(`authorization: ${session.token}`)
      session.blackbox && console.log(`blackbox: ${session.blackbox}`)
    } else {
      $.subt = `获取会话: 失败！`
      console.log(`${$.name}, ${$.subt}`)
    }
  ...
  return new Promise((resolve) => {
    ...
    $.post(url, (err, resp, data) => {
      try {
        ...
        let title = ''
        if (res.Code == 1) {
          title = `🎉当前积分: ${res.IntegralNumber}分 可抵现💰:${res.IntegralNumber / 100}元`
        } else {
          title = `❌积分查询失败`
        }
        resolve(title)
      } catch (error) {
        resolve(`❌积分查询失败 - JSON解析异常`)
      }
    })
  })
}
